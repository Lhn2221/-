...
# -------------------------------------------------------------
# TAB 2: 지출 상위 지역 범프 차트 (연속성 강조 버전)
# -------------------------------------------------------------
with tab2:
    ...
    - **시각화 방식**: 3개년 동안 지출 상위 4위를 지킨 지역들의 순위 변동 추이(Bump Chart)를 그려 이들의 독점적 연속성을 강조합니다.
    """)

    # 🌟 [강조 가시성 패널] 모든 해에 동일한 상위 지역이 유지됨을 텍스트 카드로 시각적 강조
    st.info("""
    📢 **핵심 발견 (3개년 부동의 철옹성)**  
    분석 대상 기간(**2023년 ~ 2025년**) 동안 강원도 내 지출 상위 4개 지역은 **[강릉, 속초, 춘천, 평창]**으로 단 한 번도 바뀌지 않았습니다!  
    타 지역이 이 상위권 그룹에 진입하지 못했으며, 오직 이 4개 지역 내부에서만 미세한 순위 다툼(엎치락뒤치락)이 일어났을 뿐입니다.
    """)
    ...
    if not df2.empty:
        # [개선 핵심]: 연속성과 순위 다툼을 한눈에 보여주는 '범프 차트(선 그래프)' 적용
        fig2 = px.line(
            df2,
            x='연도',
            y='순위',
            color='지역',
            markers=True,          # 데이터 포인트마다 동그란 점 표시
            title="🏆 3개년 연속 Top 4 지역의 부동의 순위 변동 추이 (범프 차트)",
            labels={"순위": "순위", "연도": "연도", "지역": "지역"},
            hover_data={"비율": ":.2f%"}  # 마우스 오버 시 실제 지출 비율(%)도 소수점 둘째자리까지 노출
        )
        
        # 1위가 그래프 맨 위로 가도록 Y축 반전 및 정수 표현 설정
        fig2.update_yaxes(
            autorange="reversed",
            tickvals=[1, 2, 3, 4],
            ticktext=["1위🥇", "2위🥈", "3위🥉", "4위"]
        )
        
        # 연도 X축 눈금을 정수 형태로 깨끗하게 표시
        fig2.update_xaxes(
            tickvals=df2['연도'].unique()
        )
        
        # 선의 두께를 늘려 가시성을 더 확보하고 디자인 정돈
        fig2.update_traces(
            line=dict(width=4),
            marker=dict(size=10)
        )
        ...
        fig2.update_layout(
            hovermode="x unified",
            legend_title_text="부동의 Top 4 지역"
        )
