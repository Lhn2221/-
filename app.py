...
# -------------------------------------------------------------
# TAB 2: 지출 상위 지역 연도별 도넛 차트 (100% 완성형 버전)
# -------------------------------------------------------------
with tab2:
    ...
    - **시각화 방식**: 매년 지출 상위 4위를 차지한 고정 멤버들과 그 외 **'타 지역' 전체를 회색으로 합쳐 온전한 100% 도넛 차트**로 표현합니다. 이를 통해 전체 지출 대비 상위 4개 지역의 압도적인 비중을 한눈에 비교합니다.
    """)
    ...
    회색으로 표시된 **'타 지역(나머지 모든 지역의 합)'**과 비교했을 때, 단 4개의 지역이 강원도 전체 지출의 절반에 가까운 엄청난 비중을 차지하고 있음을 볼 수 있습니다.
    """)
    ...
    if not df2.empty:
        # 🛡️ 방어적 코드: 만약 데이터베이스의 비율이 0~1 사이의 소수(Fraction) 형태라면 100을 곱해 백분율로 일치시킵니다.
        if df2['비율'].max() <= 1.0:
            df2['비율'] = df2['비율'] * 100.0
        ...
        # '타 지역'은 강조가 없는 연한 회색(#E0E0E0)으로 지정하여 상위 4개 지역만 돋보이게 만듭니다.
        color_map = {
            "강릉": "#FF6B6B",    # 연한 빨강
            "속초": "#4D96FF",    # 연한 파랑
            "춘천": "#6BCB77",    # 연한 초록
            "평창": "#FFD93D",    # 연한 노랑
            "타 지역": "#E5E5E5"  # 중립적인 연한 회색 (배경화 처리)
        }
        
        for i, year in enumerate(years):
            # 해당 연도의 상위 4개 지역 데이터 필터링
            df_year = df2[df2['연도'] == year].copy()
            
            # 상위 4개 지역의 비율 합산
            sum_ratios = df_year['비율'].sum()
            
            # 100%에서 상위 4개 합을 뺀 '타 지역' 비율 계산 (음수 방지 안전망 포함)
            other_ratio = max(0.0, 100.0 - sum_ratios)
            
            # '타 지역' 행(DataFrame) 데이터 생성
            ...
            # 기존 상위 4개 데이터에 '타 지역' 행 합치기 (최종 100% 데이터셋 완성)
            ...
            # 디자인 다듬기 (마지막 차트에만 범례를 표시하여 깔끔함 극대화)
            fig.update_layout(
                showlegend=(i == 2), 
